...
do i=1,(L*M)
  ...
  if ( (mod(s -1 , Lx) + 1 - Lx0)**2 + ((s-1) / Lx + 1 - Ly0)**2  .ge. find_range2 ) then
...
integer :: i, j, k, ia, s, mem, act, find_range_int
...
! Here I cycle over s', mem, a' states
    ...
    s_not_found = ( (mod(s-1,Lx)+1 - Lx0)**2 + ((s-1)/Lx+1 - Ly0)**2 .ge. find_range2)
    ...
    if (s_not_found .eqv. .true.) then
        ...
        do k=1,M
            ...
            ! old s = s + 1
                ...
                if ((mod(s,Lx)+1-Lx0)**2 + ((s)/Lx+1 - Ly0)**2 .ge. find_range2 ) &
            ...
            ! a = A*(mem-1) + 2
            ! old s = s - 1
                if ( (mod(s-2, Lx)+1 - Lx0)**2 + ((s-2)/Lx+1 - Ly0)**2 .ge. find_range2 ) &
            ...
            ! old s = s - Lx
                ...
                if ( (mod(s-Lx -1 , Lx) + 1 - Lx0)**2 + ((s-Lx-1) / Lx + 1 - Ly0)**2 .ge. find_range2 ) &
...
! Here I cycle over s, a states -> memory is free
    ...
    if ( (mod(s -1 , Lx) + 1 - Lx0)**2 + ((s-1) / Lx + 1 - Ly0)**2 .ge. find_range2) then
        ...
        if ( (mod(new_s -1 , Lx) + 1 - Lx0)**2 + ((new_s-1) / Lx + 1 - Ly0)**2 < find_range2) &
            r = r + reward_find
    endif
